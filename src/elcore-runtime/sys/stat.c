...
int _syscall_stat(const char *fildes, stat_compat *st);
...
int stat(const char *file, struct stat *st) {
    ...
    int ret = _syscall_stat(file, st_compat);
    if (ret < 0) {
        errno = -ret;
        ret = -1;
    } else
    ...
    st->st_atime = st_compat->st_atime;
    st->st_mtime = st_compat->st_mtime;
    st->st_ctime = st_compat->st_ctime;
    free(st_compat);
    return ret;
}
