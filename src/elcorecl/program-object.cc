...
template <typename T>
static ecl_program
CreateProgramWithBinaryT(ecl_context context, ecl_uint num_devices,
                        const ecl_device_id* device_list, const size_t* lengths,
                        const unsigned char** binaries, ecl_uint num_shared_sections,
                        const ecl_shared_section *shared_sections, ecl_int* errcode_ret) {
    ecl_int result;

    boost::intrusive_ptr<_ecl_programT<T>> program(new _ecl_programT<T>(context, num_devices,
                                                                device_list));

    std::vector<typename T::SharedSection> elcore_shared_sections_list;
    ...
    for (size_t i = 0; i < num_shared_sections; i++) {
        ...
        ecl_ulong paddr = reinterpret_cast<__u64>(mem->m_host_ptr);
        ...
        typename T::SharedSection elcore_shared_section;
        elcore_shared_section.mapper_fd = mem->mapper_fd;
    ...
    std::shared_ptr<typename T::parser> parser;
    try {
        parser = T::CreateELFParser(binaries[0], lengths[0]);
    } catch(const std::runtime_error& e) {
        if (errcode_ret) *errcode_ret = ECL_INVALID_BINARY;
        return nullptr;
    }

    program->m_elf = T::ELF::CreateELF(parser);
    std::vector<std::shared_ptr<_ecl_device<T>>> &devices = context->getDevices<T>();
    try {
        for (ecl_uint dev = 0; dev < num_devices; dev++) {
            std::shared_ptr<typename T::Job> job;
            if (num_shared_sections)
                job = T::CreateJobWithSharedSections(
                            parser, program->m_elf, devices[dev]->m_fd,
                            program->get_stack_size(),
                            elcore_shared_sections_list);
            else
                job = T::CreateJob(parser, program->m_elf, devices[dev]->m_fd,
                                                 program->get_stack_size());
    ...
    try {
        for (ecl_uint dev = 0; dev < num_devices; dev++) {
            ...
            program->m_job_instance_templates[dev].launcher_virtual_address =
                program->m_elf->GetKernelVirtualAddress(T::job_launcher().c_str());
            strcpy(program->m_job_instance_templates[dev].name, T::job_launcher().c_str());
            ...
            T::fix_job_instance(program->m_job_instance_templates[dev]);
    ...
    for (ecl_uint dev = 0; dev < num_devices; dev++) {
        ...
        try {
            int status = T::JobInstancePoll(job_instance_fd, devices[dev]->m_fd);
            if (status != T::JOB_STATUS_SUCCESS) {
                if (errcode_ret) *errcode_ret = ECL_INVALID_BINARY;
                return nullptr;
            }
        } catch(const std::runtime_error& e) {
...
CreateProgramWithBinary(ecl_context context, ecl_uint num_devices,
    ...
    if (context->getPlatform() == 0)
        return CreateProgramWithBinaryT<elcore50>(context, num_devices, device_list, lengths,
            binaries, num_shared_sections, shared_sections, errcode_ret);
    return CreateProgramWithBinaryT<risc1>(context, num_devices, device_list, lengths,
        binaries, num_shared_sections, shared_sections, errcode_ret);
}
...
template <typename T>
static ecl_int clGetProgramInfoT(ecl_program program, ecl_program_info param_name, size_t param_value_size,
                  void* param_value, size_t* param_value_size_ret)
{
    ...
    switch (param_name) {
        ...
        case ECL_PROGRAM_BINARY_SIZES: {
            ...
            ECL_RETURN_GETINFO_INNER(value_size, {
                for (std::size_t i = 0; i < program->m_devices.size(); ++i) {
                    ((size_t*)param_value)[i] = m_elf->GetBinarySize();
                }
            });
        }
        case ECL_PROGRAM_BINARIES: {
            ...
            ECL_RETURN_GETINFO_INNER(value_size, {
                for (std::size_t i = 0; i < program->m_devices.size(); ++i) {
                    ((const unsigned char**)param_value)[i] = m_elf->GetBinaryData();
                }
            });
        }
        ...
        case ECL_PROGRAM_NUM_KERNELS:
            ECL_RETURN_GETINFO(size_t, m_elf->GetKernels().size());
        case ECL_PROGRAM_KERNEL_NAMES: {
            size_t size = 0;

            for (auto it = m_elf->GetKernels().cbegin();
                     it != m_elf->GetKernels().cend(); ++it) {
...
eclGetProgramInfo(ecl_program program, ecl_program_info param_name, size_t param_value_size,
    ...
    if (program->getPlatform() == 0)
        return clGetProgramInfoT<elcore50>(program, param_name, param_value_size,
            param_value, param_value_size_ret);

    return clGetProgramInfoT<risc1>(program, param_name, param_value_size,
        param_value, param_value_size_ret);
}
