...
class SyscallMetaData {
 public:
    SyscallMetaData() {
        ...
        int elcore_fd = open("/dev/risc1", O_RDWR, 0);
    }
...
void Risc1FlushPreamble() {
    static std::mutex write_guard;
    std::string &stderr_buffer = s_data.m_stderr_buffer;
    std::string &stdout_buffer = s_data.m_stdout_buffer;
...
std::string get_preamble(int device_fd) {
    ...
    if (s_data.m_corenum_write) {
        snprintf(buf_devnum, sizeof(buf_devnum), "risc1: ");
...
int write_with_preamble_buffered(const char *msg, int fd, const int size, int device_fd) {
    ...
    std::string &buffer = (fd == 1) ? s_data.m_stdout_buffer:
                                      s_data.m_stderr_buffer;
...
void Risc1SyscallHandler(int job_instance_fd, int job_fd) {
    struct risc1_message message;
    int ret = read(job_instance_fd, &message, sizeof(struct risc1_message));
    if (ret < 0) {
